...
**日期**: 2026-03-07
**主题**: 环境适配、语言本地化支持及 AI 远程能力赋能
**作者**: AI Agent (Antigravity) 协助开发者编写

## 1. 宿主环境兼容性修复：Node.js 24 与 原生模块
### 现象
项目在 Antigravity (VS Code 1.97 基座, Node 24) 下启动 F5 调试即刻闪退。
### 解决方案
- **原生模块降级**：确认为 `ssh2@1.17.0` 在高版本 Node 下的 `SIGSEGV`。通过 `package.json` 的 `overrides` 字段硬锁定 `ssh2` 为 `1.15.0`。
- **文件系统适配**：针对 **exFAT** 分区不支持 `symlink` 的问题，建立规范：在移动设备挂载区分区开发时，必须使用 `npm install --no-bin-links`，确保环境稳定。

## 2. 深度本地化：中文路径解码与多格式复制
### 现象
开发者在远程服务器使用中文目录（如 `公共/whm/`）时，复制的路径呈现为 URL 码，不利于 AI 识读，且缺乏直接可用的远程 Host 地址。
### 解决方案
- 修改 `extension.ts` 中的 `svc.copyRemotePath` 命令。
- 利用 `targetUri.path` 自动解码能力，将 `%xx` 还原为直观的中文字符。
- 引入 `QuickPick` 菜单，提供 `host:path`、`fullPath`、`svc://` 等四种格式选项，适配 AI 助手和终端命令行等多种使用场景。

## 3. 监控面板升级：实时 CPU/GPU/MEM 状态
### 需求项
用户不希望只看到连接成功的提示，更需要实时洞察服务器的“心跳”。
### 实现
- 扩充 `ServerMonitorProvider`，通过 SSH 执行 `top`/`free`/`nvidia-smi` 周期性抓取数据。
- 重构 `svc.status` 命令，将其从简单的连接列表变更为详尽的**系统资源报告**。
- 支持多个 NVIDIA GPU 的并列显示（显存、利用率、温度），为深度学习任务提供便利。

## 4. 赋予 AI “跨越时空”的能力：MCP SSH 集成
### 核心突破
通过在本地部署项目私有的 `.mcp/ssh-server`（基于 `@idletoaster/ssh-mcp-server`），并手动配置 Antigravity 的 `mcp_config.json`，实现了 AI 助手与 SVC 插件连接的服务器之间的“打通”。
### 排除故障
- 修复了 `autoRegisterSSHManager` 不会自动创建目录的 Bug。
- 解决了从 HTTP 型 MCP 到标准 Stdio 型 MCP 的协议切换。
现在，AI 助手可以直接通过 SSH 并在你本地密钥的授权下，直接读取和修改远程服务器代码，极大提升了远程开发的自动化程度。

---
*总结：从解决“起步就崩”的致命错误，到实现“AI 见所未见”的远程协作，今天的进步标志着 SVC 插件已经从一个简单的连接工具，进化为了一个具备环境感知和 AI 协同能力的远程开发工作站。*
